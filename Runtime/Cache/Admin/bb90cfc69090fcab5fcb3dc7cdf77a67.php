...
<html lang="cn">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form action="<?=U('ztglobject/zzzbindex')?>" method="POST" class="search-form">
                        <label>项目编号
                            <input type="text" class="search-input" name="object_no" value="<?php echo I('object_no');?>">
                        </label>
                        <label>项目名称
                            <input type="text" class="search-input" name="object_name" value="<?php echo I('object_name');?>">
                        </label>
                        <label>
                            <button class="btn btn-sm btn-primary" type="button" id="search-btn" url="<?php echo U('ztglobject/zzzbindex');?>">
 ...
 $status = ['CRT'=>'编写中','PUB'=>'正在招标','RVK'=>'撤回招标','UPT'=>'正在更新','ADA'=>'废标','FLS'=>'流标','WBO'=>'等待开标','REV'=>'评标中','SEL'=>'施工中','END'=>'已结束']; ?>
            ...
            <table class="table table-striped table-bordered table-hover dataTable">
			    ...
			    <tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><a href="<?php echo U('viewzb',array('object_id'=>$vo['object_id']));?>"><?php echo ($vo["object_id"]); ?></a></td>
                        ...
                        <td><?php echo ($vo["bid_open_date"]); ?></td>
                        <td><?= $status[$vo['object_status']]?></td>
                        <td>
                            <?php if($vo['object_status']=='PUB'||$vo['object_status']=='WBO'||$vo['object_status']=='PUB'){ ?>
                            <a href="<?php echo U('cancel?object_id='.$object_id);?>" >
                                撤销
                            </a>
                            <?php }else{ ?>
                                <a href="<?php echo U('viewzb',array('object_id'=>$vo['object_id']));?>" >
                                    查看
                                </a>
                            <?php }?>
                        </td>
    ...
    <script type="text/javascript" src="/Public/static/city.js"></script>
    <script>
