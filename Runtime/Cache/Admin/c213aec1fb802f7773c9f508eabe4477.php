...
<html lang="cn">
    ...
    <?php  echo ace_group(ace_label('行为名称'),'<div class="line-height-2 blue">'.get_action($info['action_id'], "title").'</div>'); echo ace_group(ace_label('执行者'),'<div class="line-height-2 blue">'.get_nickname($info['user_id']).'</div>'); echo ace_group(ace_label('执行IP'),'<div class="line-height-2 blue">'.long2ip($info['action_ip']).'</div>'); echo ace_group(ace_label('执行时间'),'<div class="line-height-2 blue">'.date('Y-m-d H:i:s',$info['create_time']).'</div>'); echo ace_textarea(array('label_text'=>'备注'),'',$info['remark']); ?>
		<!-- 按钮 -->
