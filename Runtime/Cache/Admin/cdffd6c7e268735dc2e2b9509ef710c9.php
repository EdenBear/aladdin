    ...
    <div id="auth_groups">
        <form class="form-horizontal" action="<?php echo U('addToGroup');?>" method="post" enctype="application/x-www-form-urlencoded" >
			<div class="form-group">
			     <label class="col-xs-12 col-sm-12 no-padding-right"><?php echo ($nickname); ?> 所属的用户组列表</label>
            </div>
            <?php if(is_array($auth_groups)): $i = 0; $__LIST__ = $auth_groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label>
                    <input type="checkbox" value="<?php echo ($vo["id"]); ?>" name="group_id[]" class="ace ids auth_groups">
                    <span class="lbl"></span> <?php echo ($vo["title"]); ?> 
