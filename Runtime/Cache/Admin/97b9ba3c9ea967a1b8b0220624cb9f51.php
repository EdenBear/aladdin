        ...
        <div class="dataTables_wrapper">  
			...
			<table class="table table-striped table-bordered table-hover dataTable">
				...
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
                            <label>
                                <input type="checkbox" class="ace ace-switch ace-switch-6 ajax-get" name="status" value="<?php echo ($vo["status"]); ?>" <?=$vo['status'] == '1' ? 'checked' : ''?> url="<?php echo U('model/setstatus?ids='.$vo['id'].'&status='.abs(1-$vo['status']));?>">
                                <span class="lbl"></span>
                            </label>
                        </td>
                        <td>
                            <a title="删除" href="<?php echo U('model/del?ids='.$vo['id']);?>" class="ui-pg-div ui-inline confirm ajax-get">
                                删除
                            </a>
                            <?php if($vo['need_datalist']):?>
                                <a title="数据列表" href="<?php echo U('think/lists?model='.$vo['name']);?>" class="ui-pg-div ui-inline">
                                    数据列表
                                </a>
                            <?php endif;?>
                        </td>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        <label>
                            <a class="btn btn-white" href="<?php echo U('Model/add');?>">
                                新增
                            </a>
                        </label>
                        <label>
                            <a class="btn btn-white" href="<?php echo U('Model/generate');?>">
                                生成
                            </a>
                        </label>
                        <label>
                            <button class="btn btn-white ajax-post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>1));?>">
                                启 用
                            </button>
                        </label>
                        <label>
                            <button class="btn btn-white ajax-post" target-form="ids" url="<?php echo U('Model/setStatus',array('status'=>0));?>">
                                暂停
                            </button>
                        </label>
                    </div>
