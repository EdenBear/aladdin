...
<html lang="cn">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form class="search-form">
                        <label>项目名称
                            <input type="text" class="search-input" name="object_name" value="<?php echo I('object_name');?>" placeholder="请输入标的名称">
                        </label>
                        <label>状态
                            <select name="status">
 ...
 foreach ($status as $k=>$v){ $selected = $k==I('status')?'selected':''; echo '<option value="'.$k.'" '.$selected.' >'.$v."</option>"; } ?>
                        ...
                        <label>
                            <button class="btn btn-sm btn-primary" type="button" id="search-btn" url="<?php echo U('payment');?>">
            ...
            <table class="table table-striped table-bordered table-hover dataTable">
                ...
                <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                            <td>
                                <a href="<?php echo U('paymentshow?id='.$item['id']);?>">
                                <?php echo ($item["object_name"]); ?>
                                <br>
                                (<?=$item['project_id']?>)
                                </a>
                            </td>
                            <td class="text-right"><?php echo (price_format($item["total_amount"])); ?></td>
                            <td><?php echo ($item["total_period"]); ?></td>
                            <td class="text-right"><?php echo (price_format($item["left_amount"])); ?></td>
                            <td><?php echo ($item["left_period"]); ?></td>
                            <td><?php echo ($item["current_period"]); ?></td>
                            <td class="text-right"><a href="<?php echo U('paymentshow?id='.$item['id']);?>"><?php echo (price_format($item["amount"])); ?></a></td>
                            ...
                            <td>
                                <a class="ace-thumbnails" href="<?=imageView2($item['voucher_pic'])?>">
                                    <?php echo ($item["voucher"]); ?>
                                </a>
                            </td>
                            <td><?php echo $status[$item['status']];?></td>
                            <td>
                                <?php if($item['status'] == 'CRT'):?>
                                <a href="<?php echo U('paymentcheck?id='.$item['id']);?>">审核</a>
                                <?php endif;?>
                            </td>
    ...
    <link rel="stylesheet" href="/Public/Ace/css/colorbox.css" />
<script src="/Public/Ace/js/jquery.colorbox-min.js"></script>
<script>
    $(function () {
        var colorbox_params = {
            reposition:true,
