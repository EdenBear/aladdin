            ...
            <div class="row">
                <div class="col-sm-12">
                    <form action="<?=U('pfcapital/feeindex')?>" method="POST" class="search-form">
                        ...
                        <label>订单号
                            <input type="text" class="search-input" name="order_id" value="<?php echo I('id');?>">
                        </label>
                        <label>
                            <button class="btn btn-sm btn-primary" type="button" id="search-btn" url="<?php echo U('pfcapital/feeindex');?>">
            ...
            <table class="table table-striped table-bordered table-hover dataTable">
			    ...
			    <tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <!--<td>
                            <a title="查看详情" href="<?php echo U('viewdanbao?id='.$vo['id']);?>" class="ui-pg-div"><span class="ui-icon icon-zoom-in blue"></span></a>
                        </td>-->
						<td><?=$vo['id']?></td>
						<td><?php echo ($vo["object_no"]); ?>/<?=$vo['object_name']?></td>
