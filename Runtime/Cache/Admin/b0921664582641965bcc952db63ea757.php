...
<div class="tabbable">
    ...
    <?php
 $tabs = parse_config_attr($model['field_group']); if(count($tabs) > 1): ?>
    <ul class="nav nav-tabs padding-18">
        <?php if(is_array($tabs)): $i = 0; $__LIST__ = $tabs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><li <?php if(($key) == "1"): ?>class="active"<?php endif; ?>><a data-toggle="tab" href="#tab<?php echo ($key); ?>"><?php echo ($group); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
    <?php endif;?>
    <!-- 表单 -->
                ...
                <?php if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-group cf">
                    ...
                    <div class="col-xs-12 col-sm-6">
                        <?php switch($field["type"]): case "num": ?><input type="text" class="width-100" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            ...
                            <?php case "date": if(strpos($data['field'],'int') !== false):?>
                                    <input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 date" value="<?php echo (date('Y-m-d',$data[$field['name']])); ?>" placeholder="请选择日期" />
                                <?php else:?>
                                    <input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 time" value="<?php echo ($data[$field['name']]); ?>" placeholder="请选择时间" />
                                <?php endif; break;?>
                            <?php case "datetime": if(strpos($data['field'],'int') !== false):?>
                                    <input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 time" value="<?php echo (date('Y-m-d H:i',$data[$field['name']])); ?>" placeholder="请选择时间" />
                                <?php else:?>
                                    <input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 time" value="<?php echo ($data[$field['name']]); ?>" placeholder="请选择时间" />
                                <?php endif; break;?>
                            ...
                            <?php case "select": ?><select  class="width-100" name="<?php echo ($field["name"]); ?>">
                                    <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($data[$field['name']]) == $key): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select><?php break;?>
                            ...
                            <?php case "picture": ?><div class="controls" id="picture_<?php echo ($field["name"]); ?>">
									...
									<?php if(!empty($data[$field['name']])): ?><div class="upload-pre-item"><img width="120" src="<?=($field['extra'] == 'src' ? imageView2($data[$field['name']]) : get_cover($data[$field['name']]))?>"/></div><?php endif; ?>
									</div>
								</div>
								<script type="text/javascript">
								<?php
 $thumb = array(); foreach (explode("\r\n", $field['extra']) as $k=>$v){ if(empty($v)){ continue; } $thumb[] = "'thumb[$k]':'$v'"; } ?>
								//上传图片
							    /* 初始化上传插件 */
                                ...
                                $(function(){
                                    layer.ready(function(){
                                        layer.photos({
                                            photos: "#picture_<?php echo ($field["name"]); ?>"
                                        });
                                    });
                                })
								function uploadPicture<?php echo ($field["name"]); ?>(file, data){
							        ...
							        if(data.status){
                                        $("#cover_id_<?php echo ($field["name"]); ?>").val(<?php if(($field["extra"]) == "src"): ?>data.url<?php else: ?>data.id<?php endif; ?>);
                                        $("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box')
